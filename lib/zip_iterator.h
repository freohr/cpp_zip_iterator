...
namespace utils {
    namespace internal {
        ...
        struct Dereference {
            ...
            static Ret dereference(Tuple &t) {
                return std::tuple_cat(Dereference<Tuple, n - 1>::dereference(t),
                                      std::make_tuple<std::reference_wrapper<typename std::remove_reference<CurrentVal>::type>>(*(std::get<n - 1>(t))));
            }
        };
        ...
        struct Dereference<Tuple, 1> {
            ...
            static Ret dereference(Tuple &t) {
                return std::make_tuple<std::reference_wrapper<typename std::remove_reference<Val>::type>>(*(std::get<0>(t)));
            }
        };
